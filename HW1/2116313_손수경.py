...
train['family'] = train['no_of_adults'] + train['no_of_children']
train = train.drop(['no_of_adults'], axis=1)
...
test['family'] = test['no_of_adults'] + test['no_of_children']
test = test.drop(['no_of_adults'], axis=1)
...
scaler = MinMaxScaler()
train[minmax_features] = scaler.fit_transform(train[minmax_features])
test[minmax_features] = scaler.transform(test[minmax_features])
...
# RobustScaler
scaler = RobustScaler()
train[robust_features] = scaler.fit_transform(train[robust_features])
test[robust_features] = scaler.transform(test[robust_features])
...
knn_model = KNeighborsClassifier(
    metric='manhattan', n_neighbors=32, weights='distance')
...
real_test = test[feature_names]
y_pred = knn_model.predict(real_test)
print(y_pred)
sample = pd.DataFrame()
